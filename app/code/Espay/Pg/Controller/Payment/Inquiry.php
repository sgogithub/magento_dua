...
class Inquiry extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $title = $config->getValue('payment/espay/title', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        ...
        if($req_password === $password){
          ...
          if($req_signature === $gen_signature){
            ...
            $_order_id = $req_orderid;
            $quote = $om->create('Magento\Sales\Model\Order')->load($_order_id);
            if(!empty($quote->getRealOrderId())){
              // Step 4: Has order been processed?
              if($quote->getStatus() === $default_order_status){
                // Populate Order Details
                $transaction_details = array();
                $transaction_details['order_id'] = $_order_id;
                ...
                try {
                    ...
                    $trxDate = date('d/m/Y H:i:s', strtotime($quote->getCreatedAt()));
                    $formattedAmount = number_format($totalPrice, 2, '.', '');
                    $realOrderId = $quote->getRealOrderId();

                    echo "0;Success;$realOrderId;$formattedAmount;IDR;Payment For $realOrderId;$trxDate";
